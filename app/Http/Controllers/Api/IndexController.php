...
//        $videoUrl = $video[array_rand($video)];
        $videoUrl =  $url . "/video/video3.mp4";
        $officialNews = NewD::query()->where('key_name', 'guanfang')->select(['name'])->first();
        $povertyAlleviation = NewD::query()->where('key_name', 'fupin')->select(['name'])->first();
